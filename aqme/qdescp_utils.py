...
def calculate_morfeus_descriptors(final_xyz_path,self,charge,mult):
    """
    ...
    morfeus_data = morfeus_local_descps(self, elements, coordinates, morfeus_data)
...
def morfeus_local_descps(self, elements, coordinates, morfeus_data):
    """
    Calculate MORFEUS local descriptors that do not come from xTB
    """
...
def find_level_names(df_level, level):
    ...
    # fixed descriptors
    descriptors_denovo = ['code_name', 'SMILES'] + get_descriptors('denovo')['mol']

    # other descriptors
    atom_suffixes = get_descriptors('denovo')['atoms'] # de novo atom descriptors suffixes (without prefix)
    if level == 'interpret': # interpret descriptors
        atom_suffixes += get_descriptors('interpret')['mol'] + get_descriptors('interpret')['atoms']
