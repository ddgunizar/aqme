...
#  Auxiliary function that is currently not being used in this version. However, it is retained for future decisions.
...
def calculate_global_CDFT_descriptors(self, file, file_Nminus1, file_Nminus2, file_Nplus1, file_Nplus2, xtb_morfeus_descriptors):
    ...
    electrodonating_chemical_potential, electroacepting_chemical_potential = None, None
    maximum_electron_flow, electrodonating_maximum_electron_flow = None, None
    ...
    maximum_electron_flow = round((-(chemical_potential/chemical_hardness)),4)
    electrodonating_chemical_potential = round(((1/4)*((-3*delta_SCC_IP) - delta_SCC_EA)),4)
    electroacepting_chemical_potential = round(((1/4)*(delta_SCC_IP + (3*delta_SCC_EA))),4)
    ...
    cdft_descriptors = {
        ...
        "max. electron flow": maximum_electron_flow,
        "Electrodon. Chem. potential": electrodonating_chemical_potential,
        "Electroacep. Chem. potential": electroacepting_chemical_potential,
...
def calculate_local_CDFT_descriptors(self, file_fukui_N, file_fukui_Nplus1, file_fukui_Nminus1, cdft_descriptors, xtb_morfeus_descriptors):
    ...
    for file in [file_fukui_N, file_fukui_Nplus1, file_fukui_Nminus1]:
        if not os.path.exists(file):
            self.args.log.write(f"x  WARNING! The file {file} does not exist.\n")
            return None
    ...
    # Read Fukui JSONs
    fukui_files = [
        str(os.path.dirname(file)) + "/fukui_N.json",
        str(os.path.dirname(file)) + "/fukui_Nplus1.json",
        str(os.path.dirname(file)) + "/fukui_Nminus1.json"
    ]

    def read_fukui_json(json_file):
        try:
            # Limpia posibles líneas sueltas con solo coma
            ...
            # read json
            json_data = read_json(json_file)
            return json_data.get('partial charges', [])
        except Exception as e:
            self.args.log.write(f"x  WARNING! Error reading {json_file}: {e}\n")
            return []
        finally:
            if os.path.exists(json_file):
                os.remove(json_file)

    # Lee las cargas de cada uno de los archivos
    chargs_N, chargs_Nplus1, chargs_Nminus1 = [read_fukui_json(f) for f in fukui_files]

    # Check essential descriptors
    if not cdft_descriptors:
        self.args.log.write("x  WARNING! Missing required CDFT descriptors.\n")
        return None
    ...
    s_negs = [round(chemical_softness * f_n, 4) for f_n in f_negs]
    s_rads = [round(chemical_softness * f_r, 4) for f_r in f_rads]

    # Relative descriptors
    Relative_nucleophilicity = [round(s_n / s_p, 4) if s_p != 0 else None for s_n, s_p in zip(s_negs, s_pos)]
    Relative_electrophilicity = [round(s_p / s_n, 4) if s_n != 0 else None for s_n, s_p in zip(s_negs, s_pos)]

    # Grand Canonical Dual Descriptor
    ...
    Nu_pos = [round(nucleophilicity_index * f_p, 4) for f_p in f_pos]
    Nu_negs = [round(nucleophilicity_index * f_n, 4) for f_n in f_negs]
    Nu_rads = [round(nucleophilicity_index * f_r, 4) for f_r in f_rads]
    ...
    localDescriptors = {
        ...
        "Nu_Nucleophil.": Nu_negs,
        "Nu_Radical attack": Nu_rads
    }
...
def calculate_global_morfeus_descriptors(final_xyz_path,self):
    ...
    try:
        ...
        return {
            "Global SASA area": sasa_area_global,
            "Global SASA volumen": sasa_volumen_global,
    ...
    try:
        ...
        sasa_area_global = round(sasa.area, 4)  # Assign the calculated SASA area value
        sasa_volumen_global = round(sasa.volume, 4) # Assign the calculated SASA volumen value
    except Exception as e:
        # Handle any errors during SASA calculation
        self.args.log.write(f"x  WARNING! Error calculating SASA from Morfeus: {e}")

    # Calculate Global Dispersion Area, Volume and 🍺P_int
    try:
        ...
        disp.compute_p_int()
        p_int_global = round(disp.p_int, 4)
    except Exception as e:
        # Handle any errors during Dispersion calculation
        self.args.log.write(f"x  WARNING! Error calculating Dispersion from Morfeus: {e}")
...
def get_descriptors(level):
    ...
    descriptors = {
        'denovo': {
            'mol': ["HOMO-LUMO gap", "HOMO", "LUMO", "IP", "EA", "Dipole module", "Total charge", "Global SASA area", "Global SASA volumen", "G solv. in H2O", "G of H-bonds H2O"],
        ...
        'full': {
            'mol': ["HOMO-LUMO gap_GFN", "HOMO_GFN", "LUMO_GFN", "Dipole module_GFN", "Total energy", "Total disp. C6", "Total disp. C8",
                     "Dispersion area", "Dispersion volume", "Dispersion P_int", "Chem. potential", "Electrodon. power idx", "Electroaccep. power idx",
                    "max. electron flow", "Electrodon. Chem. potential", "Electroacep. Chem. potential", "Electrodon. max. electron flow",
